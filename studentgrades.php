...
if (empty($userid)) { 
    ...
    <a onclick="javascript: sendSMS('<?php echo $other_details2->data; ?>','<?php echo @$userdetails->firstname.' '.@$userdetails->lastname; ?>');" class="button-success" href="#">Send SMS</a></div>
    <div style="padding-left:10px;float:right; padding-top:5px;"><a class="button-success" href="mailto: <?php echo @$userdetails->email; ?>">Send Email</a></div>
    <?php } echo '<div class="styled">
    ...
    print_grade_page_head($courseid, 'report', 'overview', get_string('pluginname', 'gradereport_overview') . ' - ' . fullname($report->user), false, false, false, null, null, $report->user);
    //  groups_print_course_menu($course, $gpr->get_return_url('studentgrades.php?id='.$courseid, array('userid'=>0)));
	groups_print_course_menu($course, $gpr->get_return_url('studentgrades.php'));
    ...
    if ($user_selector) {
        ...
        if($if_student==0) { 
            echo '<form name="search" autocomplete="off" action="" method="POST">
            <table ><tr><td><div class="autocomplete" style="width:220px;">
            <input id="studentname" type="text" name="studentname" placeholder="Type your Student Name" value="" />
            <input id="studentid" type="hidden" name="studentid" value="" />
        ...
        if($if_student==0) {
            echo '<div style="padding-left:10px;float:right; padding-top:5px;">'; ?> <a onclick="javascript: sendSMS('<?php echo $other_details2->data; ?>','<?php echo @$userdetails->firstname.' '.@$userdetails->lastname; ?>');" class="button-success" href="#">Send SMS</a></div>
            <div style="padding-left:10px;float:right; padding-top:5px;"><a class="button-success" href="mailto: <?php echo @$userdetails->email; ?>">Send Email</a></div> <?php 
        } ?>
	...
	as gg LEFT JOIN {grade_items} as gi ON gg.itemid = gi.id
        ...
        $sql_other_quiz_result = "
            SELECT
                gg.id               AS rowid,
                gi.itemname,
                gi.itemmodule,
                gi.iteminstance     AS quizinstance,
                gg.finalgrade,
                gg.timemodified
            ...
            AND gi.itemname   LIKE :compulsory
        ";
        $params = [
            'courseid'   => $item->courseid,    // note: use $item from step 3
            'userid'     => $userid,
            'compulsory' => '%Compulsory%',
        ];
        $list_result_quiz = $DB->get_records_sql($sql_other_quiz_result, $params);
...
//print_r($list); 
        ...
        {
            $sql_module = "SELECT id , deletioninprogress FROM {course_modules} WHERE course = '".$item->courseid."' AND instance = '".$item->assignmentid."'";
            $list_module = $DB->get_record_sql($sql_module);
            //	echo '<pre>';
            //	print_r($list_module);
            if($list_module->deletioninprogress==0)
            {
                if($item->scaleid>0 && $item->gradetype!=1)
                {
                    if(@$contextid->id!='' && @$contextid->id>0)
                    {
                        $sql_role = "SELECT roleid FROM {role_assignments} WHERE contextid = '".$contextid->id."' AND userid = '".$item->userid."'";
                        $list_all_stu = $DB->get_record_sql($sql_role);
                        
                        
                        if($list_all_stu->roleid!='' && $list_all_stu->roleid==5)
                        {
                            $grade_val = intval($item->recorded_grade); 
                            @$scale_text = $scale_array[$item->scaleid][$grade_val];
                            if(@$scale_text=='')
                            {
                            ...
                            {
                                $timemodified = @date("F j, Y, g:i a",$item->timemodified);
                            }
                            $grade_exists = '';
                            $activitydate = @date("F j, Y, g:i a",$item->activitydate);
                            $arr[] = array('baseurl'=>$CFG->wwwroot,'rowid'=>$item->rowid,'userid'=>$item->userid, 'name'=>$item->firstname.' '.$item->lastname, 'username'=>$item->username , 'assignmentname'=>$item->assignmentname,'assignmentid'=>$item->assignmentid,'grademin'=>$item->grademin,'grademax'=>$item->grademax,'timemodified'=>$timemodified,'timemodifiedg'=>'','gradeexists'=>$grade_exists,"result"=>$scale_text,"status"=>$item->status,"feedback"=>$item->feedback,"itemidother"=>$item->itemidother,"feedbackposted1"=>@$item->feedbackposted1,"feedbackposted2"=>@$item>"feedbackposted2","moduleid"=>$list_module->id,'activitydate'=>$activitydate,'countsubmission'=>$item->countsubmission);
                            unset($grade_val);
                            unset($scale_text);
                            unset($timemodified);
                            unset($list_all_stu);
                ...
                else if($item->scaleid=='' && $item->gradetype==1)
                {
                    if(@$contextid->id!='' && @$contextid->id>0)
                    {
                        $sql_role = "SELECT roleid FROM {role_assignments} WHERE contextid = '".$contextid->id."' AND userid = '".$item->userid."'";
                        ...
                        if($list_all_stu->roleid!='' && $list_all_stu->roleid==5)
                        {
                            ...
                            else
                            {
                                ...
                                if($item->feedback!='')
                                {
                                    @$scale_text = @$scale_text." - ".strip_tags($item->feedback);
                                }
                            }
                            if($item->timemodified=='')
                            {
